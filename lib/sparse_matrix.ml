...
type 'a t   = { cols : int ;
                rows : int ;
                data : 'a col Int_map.t }

exception Out_of_bounds
exception Dimensions_mismatch

let create ~rows ~cols =
  { cols ; rows ; data = Int_map.empty }

let cols { cols ; _ } = cols
let rows { rows ; _ } = rows

let get_col_exn column (m : 'a t) : 'a col =
  Int_map.find column m.data

let get_col column (m : 'a t) : 'a col option =
  Int_map.find_opt column m.data

let set_col column (c : 'a col) (m : 'a t) : 'a t =
  { m with data = Int_map.add column c m.data }

let get_elt_exn row (c : 'a col) : 'a =
  Int_map.find row c

let set_elt row (c : 'a col) elt : 'a col =
  Int_map.add row elt c

let column_is_empty column (m : 'a t) : bool =
  not (Int_map.mem column m.data)

let row_is_empty row (m : 'a t) : bool =
  Int_map.for_all (fun _ c ->
      not (Int_map.mem row c)
    ) m.data
...
let set : row: int -> col: int -> 'a t -> 'a -> 'a t =
  ...
  if row < 0 || row >= m.rows then
    raise Out_of_bounds ;
  if col < 0 || col >= m.cols then
    raise Out_of_bounds ;
  let c =
    match get_col col m with
    | None -> Int_map.empty
    | Some c -> c in
...
let mul (m1 : float t) (m2 : float t) : float t =
  ...
  Int_map.fold (fun col2 c2 acc ->
      if column_is_empty col2 m2 then
        acc
      else
        let rows = integers rows in
        List.fold_left (fun acc row1 ->
            if row_is_empty row1 m1 then
              acc
            else
              let res =
                Int_map.fold (fun row2 elt2 acc ->
